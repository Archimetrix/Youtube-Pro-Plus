...
function injectBadgesOnPage() {
    ...
    document.querySelectorAll(THUMB_LINK_SELECTORS).forEach(thumb => {
        ...
        // IMPORTANT: do NOT use getThumbContainer()/the inner #img-yt-image
        // element as the mount point. That inner element is the same node
        // YouTube anchors its native hover-preview player to. Forcing
        // position:relative on it breaks YouTube's own preview positioning
        // math, which is what caused the preview to detach and float over
        // the sticky masthead/chip-bar while scrolling.
        //
        // Instead, mount on the stable outer `a#thumbnail` anchor, only
        // touch `position` if it isn't already positioned, and `contain`
        // our overlay to the box so nothing we add can ever visually
        // escape the thumbnail bounds.
        ...
        if (!bar) {
            ...
            bar.style.cssText = [
                ...
                'pointer-events:none', 'z-index:2',    // just needs to sit above the image, not global chrome
        ...
        if (!badge) {
            ...
            badge.style.cssText = [
                ...
                'border:1px solid rgba(0,210,255,0.45)', 'z-index:3',
                'line-height:14px'
            ].join(';');
