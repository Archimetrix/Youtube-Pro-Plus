...
(function () {
    ...
    // chrome.storage.local.get/set returns a Promise in some environments (and the
    // callback is never invoked) and requires a callback in others. Handle both so
    // this never silently hangs on any browser.
    function getStorage(keys) {
        return new Promise((resolve, reject) => {
            try {
                const maybePromise = chrome.storage.local.get(keys, (result) => {
                    if (chrome.runtime && chrome.runtime.lastError) {
                        // Swallow — some browsers set lastError even on success paths.
                    }
                    resolve(result || {});
                });
    ...
    function setStorage(obj) {
        return new Promise((resolve, reject) => {
            try {
                const maybePromise = chrome.storage.local.set(obj, () => resolve());
                if (maybePromise && typeof maybePromise.then === 'function') {
                    maybePromise.then(resolve).catch(() => resolve());
                }
            } catch (e) {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            #yt-pro-room-toggle.yt-pro-room-toggle-floating {
                position: fixed; top: 64px; right: 16px; z-index: 2200;
                margin-right: 0; background: rgba(28,28,28,0.9);
    ...
    function placeToggleButton() {
        ...
        // Fallback: the "Create" button selectors didn't match (locale, A/B
        // layout, or a YouTube markup change). Rather than never showing the
        // button, dock it directly into the masthead's end/right section so
        // it's always reachable.
        if (document.body.contains(toggleBtnEl)) return;
        const fallbackHost =
        ...
        if (fallbackHost) {
            fallbackHost.insertBefore(toggleBtnEl, fallbackHost.firstChild);
        } else {
            // Last resort: a small fixed button so it's never completely missing.
            toggleBtnEl.classList.add('yt-pro-room-toggle-floating');
    ...
    function init() {
        ...
        getStorage(['watchparty']).then(({ watchparty }) => proceed(watchparty)).catch(() => proceed(undefined));
        // Safety net: never let a storage hiccup silently prevent the button
        // from ever appearing — default to enabled if the read hasn't
        // resolved within a second.
        setTimeout(() => proceed(undefined), 1000);
    }
